...
class CompareStream final : public ICoreStream
{
public:
    CompareStream(std::unique_ptr<ICoreStream> stream1, std::unique_ptr<ICoreStream> stream2);
...
private:
    ...
    std::unique_ptr<ICoreStream> stream1_;
    std::unique_ptr<ICoreStream> stream2_;
};
...
private:
    ...
    Stream                  *stream_;
    ObjectOwnershipPolicy   streamOwnershipPolicy_;

    AlignedStreamMode   mode_;
    size_t              baseAlignment_;
