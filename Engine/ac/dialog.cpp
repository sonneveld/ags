        ...
        if (parserInput) {
          ...
          // type into the parser
            ...
            if (gkey.type == SDL_KEYDOWN) {
                ...
                if ((gkey.key.keysym.scancode = SDL_SCANCODE_SPACE) && (strlen(parserInput->text) == 0)) { repeat = true; }
            }
          if (repeat) {
            // write previous contents into textbox (F3 or Space when box is empty)
            for (size_t i = strlen(parserInput->text); i < strlen(play.lastParserEntry); i++) {
