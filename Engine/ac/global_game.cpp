...
int RunAGSGame (const char *newgame, unsigned int mode, int data) {
    ...
    spriteset.Reset();
    err = spriteset.InitFile(SpriteCache::DefaultSpriteFileName.GetCStr(), SpriteCache::DefaultSpriteIndexName.GetCStr());
    if (!err)
...
int SaveScreenShot(const char*namm) {
    ...
    if (!buffer->SaveToFile(fileName.GetCStr(), palette) != 0)
    {
        delete buffer;
        return 0;
    }
