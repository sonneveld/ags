...
void SceneGraphDriver::DrawSprite(int x, int y, IDriverDependantBitmap *bitmap) {
...
SceneGraphBitmap::SceneGraphBitmap(Common::Bitmap *bitmap, bool hasAlpha, bool opaque) {
    hasAlpha_ = hasAlpha;
    opaque_ = opaque;
    bitmap_ = std::make_unique<AGS::Common::Bitmap>();
	bitmap_->CreateCopy(bitmap);
}
...
int SceneGraphBitmap::GetWidth() { return this->bitmap_->GetWidth(); }
int SceneGraphBitmap::GetHeight() { return this->bitmap_->GetHeight(); }
int SceneGraphBitmap::GetColorDepth() { return this->bitmap_->GetColorDepth(); }
