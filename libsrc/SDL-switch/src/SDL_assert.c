...
static void SDL_ExitProcess(int exitcode)
{
    ...
    exit(exitcode);
#elif defined(__NINTENDO_SWITCH__)
    exit(exitcode);  /* !!! FIXME: C runtime doesn't have _exit()...? static destructors and atexit() will run! */
#else
    _exit(exitcode);
#endif
}
...
#elif defined(__NINTENDO_SWITCH__)
        extern SDL_assert_state SDL_NintendoSwitch_PromptAssertion(const char *message);
        state = SDL_NintendoSwitch_PromptAssertion(message);
