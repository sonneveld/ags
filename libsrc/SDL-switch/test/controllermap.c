...
LoadTexture(SDL_Renderer *renderer, const char *file, SDL_bool transparent)
{
    ...
    const size_t maxlen = SDL_strlen(path) + SDL_strlen(file) + 1;
    path = SDL_realloc(path, maxlen);
    SDL_strlcat(path, file, maxlen);
...
#if defined(__ANDROID__) || defined(__NINTENDO_SWITCH__)
