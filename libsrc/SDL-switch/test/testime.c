...
#define DEFAULT_FONT "rom:/unifont-10.0.07.hex"
#else
...
int main(int argc, char *argv[])
{
    ...
    SDL_free(SDL_GetBasePath());  /* just force this to mount the ROM filesystem. */
#endif
        ...
        while (SDL_PollEvent(&event)) {
            ...
            switch(event.type) {
                ...
                case SDL_MOUSEBUTTONDOWN: {
                    char *txt = SDL_NintendoSwitch_RunSoftwareKeyboard(SDL_NINTENDOSWITCH_KEYBOARD_SINGLELINE, "Enter some text!", 20);
                    if (txt) {
                        SDL_Event e;
                        SDL_zero(e);
                        e.type = SDL_TEXTINPUT;
                        SDL_strlcpy(e.text.text, txt, sizeof (e.text.text));
                        SDL_free(txt);
                        SDL_PushEvent(&e);
                    }
                    break;
