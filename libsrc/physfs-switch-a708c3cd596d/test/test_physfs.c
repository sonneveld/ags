...
#if defined(__NINTENDO__) && defined(__NX)
#define __NINTENDO_SWITCH__ 1
#include <nn/os.h>
#include <nn/swkbd/swkbd_Api.h>
#include <nn/swkbd/swkbd_Result.h>

static char *NintendoSwitch_RunSoftwareKeyboard(void)
{
    char *retval = NULL;
    ...
    if (nn::swkbd::ResultCanceled::Includes(rc)) {
        ...
        const char16_t* utf16 = (char16_t *) str.ptr;
        size_t numchars = 0;
        while (utf16[numchars] != 0) {
            numchars++;
        }
        const size_t utf8len = (numchars+1) *  4;
        retval = (char *) malloc(utf8len);
        if (retval) {
            PHYSFS_utf8FromUtf16((const PHYSFS_uint16*) utf16, retval, utf8len);
        }
    }
    free(outbuf);
    free(workbuf);
...
static int cmd_cat2(char *args)
{
    ...
    else
    {
        ...
        do
        {
            ...
            ptr = (char *) realloc(buffer1, buffer1len + readlen);
            if (!ptr)
            {
            ...
            ptr = (char *) realloc(buffer2, buffer2len + readlen);
            if (!ptr)
            {
                printf("(Out of memory.)\n\n");
                free(buffer1);
...
#if (defined PHYSFS_HAVE_READLINE)
        buf = readline("> ");
#elif __NINTENDO_SWITCH__
        buf = NintendoSwitch_RunSoftwareKeyboard();
#else
        int i;
        buf = (char *) malloc(512);
        memset(buf, '\0', 512);
...
#if __NINTENDO_SWITCH__
extern "C" { void nnMain(void); }
void nnMain(void)
{
    const char *argv[] = { "/nxprog", NULL };
    (void) main(1, (char **) argv);
}
#endif
